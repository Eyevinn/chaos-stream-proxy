...
export async function loadParameterFromSSMCached(parameterName: string): Promise<string> {
...
export function addSSMUrlParametersToUrl(url: string): Promise<string> {
    if (!url.includes('delay') && !url.includes('statusCode') && !url.includes('timeout') && url.includes('proxy-master')) {
        const parameterName = process.env.AWS_SSM_PARAM_KEY ?? '/ChaosStreamProxy/Development/UrlParams';
