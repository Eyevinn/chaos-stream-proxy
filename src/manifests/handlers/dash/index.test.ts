 ...
 */
/*
 import dashHandler from "../../routes/index";
 import nock from "nock";
 import { ALBEvent, ALBResult } from "aws-lambda";
 import { createReadStream, ReadStream } from "fs";
 import path from "path";
 
 describe("manifests.handlers.dasg.ts", () => {
   describe("dashHandler", () => {
     let mockBaseURL: string;
     let mockMasterURL: string;
     beforeEach(() => {
       mockBaseURL = "https://mock.mock.com/stream/dash";
       mockMasterURL = "https://mock.mock.com/stream/dash/manifest.mpd";
     });
     ...
     it("should return proxy master manifest with queryParams on Media Playlist URLs", async () => {
       const getBody = () => {
         return new Promise((resolve, reject) => {
           const readStream: ReadStream = createReadStream(path.join(__dirname, "../../../testvectors/dash/dash1_multitrack/manifest.m3u8"));
           resolve(readStream);
         });
       };
     ...
     it("should return code 400 when 'url' query parameter is missing in request", async () => {
       const getBody = () => {
         return new Promise((resolve, reject) => {
           const readStream = createReadStream(path.join(__dirname, "../../../testvectors/dash/dash1_multitrack/manifest.m3u8"));
           resolve(readStream);
         });
       };
       nock(mockBaseURL).persist().get("/manifest.m3u8").reply(200, getBody, {
     ...
     it("should return code 400 if source URL is a Media Playlist manifest", async () => {
       const getBody = () => {
         return new Promise((resolve, reject) => {
           const readStream = createReadStream(path.join(__dirname, "../../../testvectors/dash/dash1_multitrack/manifest_1.m3u8"));
           resolve(readStream);
         });
       };
       nock(mockBaseURL).persist().get("/manifest.m3u8").reply(200, getBody, {
       ...
       const expected: ALBResult = {
         ...
         body: '{"reason":"Input dash stream URL is not a Multivariant Playlist"}',
       };
       expect(response.statusCode).toEqual(expected.statusCode);
       expect(response.headers).toEqual(expected.headers);
       expect(response.body).toEqual(expected.body);
