...
describe("dashManifestTools", () => {
  describe("createProxyDASHManifest", () => {
    it("should replace initialization urls & media urls in dash manifest, with absolute source url & proxy url with query parameters respectively", async () => {
      // Arrange
      const mockManifestPath = "../../testvectors/dash/dash1_multitrack/manifest.xml";
      let mockDashManifest = fs.readFileSync(path.join(__dirname, mockManifestPath), "utf8");
      const queryString = "url=https://mock.mock.com/stream/dash/asset44/manifest.mpd&statusCode=[{i:0,code:404},{i:2,code:401}]&timeout=[{i:3}]&delay=[{i:2,ms:2000}]";
      const urlSearchParams = new URLSearchParams(queryString);
      // Act
      const manifestUtils = dashManifestUtils();
      const proxyManifest: string = manifestUtils.createProxyDASHManifest(mockDashManifest, urlSearchParams);
      // Assert
      const parser = new xml2js.Parser();
      const builder = new xml2js.Builder();
      const proxyManifestPath = "../../testvectors/dash/dash1_multitrack/proxy-manifest.xml";
      const dashFile: string = fs.readFileSync(path.join(__dirname, proxyManifestPath), "utf8");
      let DASH_JSON;
...
describe("utils.mergeMap", () => {
  it("should handle priority without default corrtupions", () => {
    ...
    mockAllCorruptions.set(mockReqSegIndex, new Map<string, CorruptorConfig>().set("a", { fields: { ms: 100 } }).set("b", { fields: { code: 300 } }));

    // Act
    const actual = dashManifestUtils().utils.mergeMap(mockReqSegIndex, mockAllCorruptions);
    const expected = new Map<string, CorruptorConfig>().set("a", { fields: { ms: 100 } }).set("b", { fields: { code: 300 } });
  ...
  it("should handle priority with default corrtupions", () => {
    ...
    for (let segIdx = 0; segIdx < 3; segIdx++) {
      actual.push(dashManifestUtils().utils.mergeMap(segIdx, mockAllCorruptions));
    }
  ...
  it("should handle empty fields prop correct", () => {
    // Arrange
    const mockReqSegIndex = 3;
    const mockAllCorruptions = new Map<TargetIndex, CorruptorConfigMap>().set(mockReqSegIndex, new Map<string, CorruptorConfig>().set("a", { fields: {} }));
    // Act
    const actual = dashManifestUtils().utils.mergeMap(mockReqSegIndex, mockAllCorruptions);
    const expected = new Map<string, CorruptorConfig>().set("a", { fields: {} });
    // Assert
    expect(actual).toEqual(expected);
  });
});
